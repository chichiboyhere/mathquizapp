...
const MeasuresOfDispersionUngrouped = () => {
  ...
  const calcMeanDeviation = (mean, sumFreq) => {
    const devFromMean = data.reduce((sum, item) => sum + (Math.abs(parseFloat(item.value)  - mean)* parseInt(item.freq)), 0)
  ...
  const calcVariance = (mean, sumFreq) => {
    const summationfxsquared = data.reduce((sum, item) => sum + ((Math.pow((parseFloat(item.value)  - mean), 2))* parseInt(item.freq)), 0)
